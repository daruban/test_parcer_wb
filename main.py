...
class WBParser:
    ...
    async def export_products_and_cards(self, query, sleep_sec=0.5):
        ...
        for product in tqdm(products[0:LIMIT_ITEMS]):
            item_data = await self.get_card(product = product)
            if item_data:
    ...
    async def import_xlsx(self, items):
        """импорт в xlsx"""
...
async def main():
    app = WBParser()
    items = await app.export_products_and_cards(QUERY)
    pars_items = await app.pars(items)
    await app.import_xlsx(pars_items)
